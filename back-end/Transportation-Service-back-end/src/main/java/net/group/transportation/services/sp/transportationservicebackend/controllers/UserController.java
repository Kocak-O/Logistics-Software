...
public class UserController {
    ...
    if (!user.isEmailVerified()){
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Please verify your Account");
    }
    ...
    private void sendVerificaiton(User user){
        String link = "http://localhost:7070/api/users/verify?token=" + user.getVerificationToken();

        SimpleMailMessage message = new SimpleMailMessage();
        message.setTo(user.getEmail());
        message.setSubject("Verificate your Account - Transport");
        message.setText("Please click the following Link to verify your Mail Adress " + link);
